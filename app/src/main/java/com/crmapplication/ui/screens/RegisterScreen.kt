...
fun RegisterScreen(
    ...
    fun submit() {
        ...
        localError = validateRegister(name, email, password, confirmPassword)
        if (localError == null) {
    ...
    AuthBackground {
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            Text("Already have an account?", color = CrmOnSurfaceVar, fontSize = 13.sp)
            TextButton(onClick = onNavigateToLogin) {
                Text("Sign In", color = CrmPrimary, fontWeight = FontWeight.SemiBold, fontSize = 13.sp)
            }
        }
    }
}
